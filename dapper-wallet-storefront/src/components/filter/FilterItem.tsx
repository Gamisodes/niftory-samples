...
export const FilterItem = ({ title, options, setFilter, index }) => {
  const [isOpen, setOpen] = useState(false)
  
  const onSelectFilter = useCallback((idx) => (event) => {
    setFilter((prev) => {
    ...
    >
      ...
      <div className="body">
        ...
        <div className="grid gap-4 pt-4">
          {options.map(({ selected, value }: { selected: boolean; value: string }, index) => (
            <label key={value + index} className="flex gap-3 align-middle">
